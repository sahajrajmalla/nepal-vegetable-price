...
The study utilises daily wholesale price records from the Kalimati Fruits \& Vegetables Market, the largest wholesale market in Kathmandu, Nepal. Two raw CSV datasets span the period from June 2013 to September 2023. Table~\ref{tab:data_sources} summarises the dataset characteristics.

\begin{table}[htbp]
\centering
\caption{Raw datasets covering the study period.}
\label{tab:data_sources}
\begin{tabular}{llr}
\toprule
\textbf{Dataset} & \textbf{Period} & \textbf{Records} \\
\midrule
File 1 & June 2013 -- May 2021 & $\sim$191,000 \\
File 2 & May 2021 -- September 2023 & $\sim$90,000 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Data Combination}
The two raw CSV files are merged via an automated script that performs schema detection, price cleaning (stripping currency prefixes and comma separators), date parsing, and unit harmonisation. Furthermore, it classifies each commodity into hierarchical categories (e.g., Root \& Tuber Vegetables $\rightarrow$ Root Vegetables) using a keyword-matching taxonomy. Exact duplicates on the composite key \texttt{[Commodity, Date, Minimum, Maximum, Average]} are dropped, yielding the unified dataset.
...
\subsubsection{Schema Normalisation and Duplicate Removal}
Price columns are cleaned to handle plain numerics and \texttt{NaN} values, whilst string-based features (units and commodity names) are standardised. Exact duplicates on \texttt{[Commodity, Date]} are dropped, retaining the last occurrence to correctly resolve overlapping recording periods.
...
\begin{equation}
\text{lower} = \max\left(Q_1 - k \cdot \text{IQR},\ P_{\min}\right), \quad \text{upper} = \min\left(Q_3 + k \cdot \text{IQR},\ P_{\max}\right)
\end{equation}
...
\begin{itemize}
    \item \textbf{Autoregressive Lags (7):} Target variable lagged at $1, 2, 3, 7, 14, 21, \text{ and } 30$ days.
    \item \textbf{Rolling Window Statistics (15):} Mean, standard deviation, minimum, maximum, and median computed over trailing windows of size $w \in \{7, 14, 30\}$ days.
    \item \textbf{Exponentially Weighted Moving Averages (3):} EWMA with span parameters $\alpha \in \{7, 14, 30\}$.
    \item \textbf{Differencing (2):} First-order (daily) and seasonal (weekly) differences.
    \item \textbf{Calendar Features (12):} Integer encodings for day of the week, month, day of the year, quarter, and week of the year. Because Nepal observes Saturday as the primary weekly holiday, a custom \texttt{is\_weekend} binary flag is implemented. Cyclical sine and cosine transformations are applied to the day of the year, day of the week, and month.
    \item \textbf{Nepal Festival Dummy Variables (8):} Binary indicators for six major Nepali festivals (Dashain, Tihar, Chhath, Holi, Teej, and Nepali New Year). These incorporate specific lead and lag windows (e.g., 7 days lead and 3 days lag for Dashain) to capture pre- and post-festival price effects. A composite \texttt{fest\_any} flag and a weekend interaction term are also included.
    \item \textbf{Price-Derived Features (7):} Metrics such as spread, price velocity, 7-day price momentum, acceleration, squared returns (serving as a volatility clustering proxy), and rolling CVs for 7, 14, and 30 days.
    \item \textbf{STL Decomposition (3):} Seasonal and Trend decomposition using Loess (STL) applied with a weekly period, extracting trend, seasonal, and residual components.
\end{itemize}

\subsection{Experimental Setup}

\subsubsection{Train/Test Split and Evaluation Horizons}
...
\subsubsection{Forecasting Strategy and Metrics}
...
\subsubsection{Reproducibility and Computational Environment}
...
\begin{enumerate}
    \item \textbf{ARIMA-LSTM:} Utilises ARIMA for linear autocorrelation extraction, followed by a ResidualLSTM architecture to model the remaining nonlinear residual structure.
    \item \textbf{ARIMA-HistGB:} Augments the ML feature matrix with ARIMA's in-sample fitted values, which are then trained via a HistGB model to capture linear and non-linear patterns concurrently.
\end{enumerate}
...
\subsubsection{Stacking Ensemble Meta-Learner}
A multi-strategy ensemble approach aggregates predictions from 12 full-length models. A Dynamic Ensemble Selection (DES) mechanism employs the first 30\% of the test set (136 days) as a validation set, strictly filtering out any model with $R^2 \leq 0$. Of the 12 candidate models, 7 were retained. An inverse-RMSE weighted average of the top 5 models (XGBoost, HistGB, ExtraTrees, RandomForest, and GRU) was selected as the optimal meta-learner strategy following a competitive evaluation of six distinct ensemble methods.
